package LeetCode.Daily_Challenge.Hard.Greatest_Divisor_Traversal;
...
public class Solution {
    ...
    public void dfs(List<List<Integer>> adj, int[] vis, int st) {
        vis[st] = 1;
    ...
    public static void helper(int n, Map<Integer, List<Integer>> map, int st) {
        while (n % 2 == 0) {
            map.put(2, map.getOrDefault(2, new ArrayList<>()));
            List<Integer> temp = map.get(2);
            temp.add(st);
            map.put(2, temp);
            n /= 2;
        }
